#include "agent/strategy/rule_based_policy.hpp"
    ...
    namespace {
        ...
        TEST(AgentObservationTest,
            ...
            const AgentObservation observed_a =
                world.observations.observe(
                    agent_a,
                    world.observations.capture_world(7));
            EXPECT_EQ(observed_a.agent_id, agent_a);
            EXPECT_EQ(observed_a.account_id, account_a);
            EXPECT_EQ(observed_a.world.step, 7U);
            EXPECT_EQ(observed_a.world.internal_market.best_bid, 90);
            EXPECT_EQ(observed_a.world.internal_market.best_ask, 100);
            EXPECT_EQ(observed_a.base_balance, (Balance{5, 2}));
            EXPECT_EQ(observed_a.quote_balance, (Balance{500, 0}));
            ASSERT_EQ(observed_a.active_orders.size(), 1U);
            EXPECT_EQ(
                observed_a.active_orders.front(),
                (ObservedOrder{1, Side::Sell, 100, 2}));
        ...
        TEST(AgentFoundationTest, UnknownAgentObservationAndActionFail) {
            ...
            EXPECT_THROW(
                static_cast<void>(world.observations.observe(
                    999,
                    world.observations.capture_world(1))),
                std::out_of_range);
            EXPECT_THROW(
                static_cast<void>(
        ...
        TEST(TradingRequestAgentExecutionAdapterTest,
            ...
            EXPECT_EQ(
                ...
                (AgentActionResult{SubmitActionResult{
                    0,
                    0,
                    AgentSubmitStatus::InvalidOrder}}));
            EXPECT_TRUE(world.ledger.entries().empty());
            EXPECT_EQ(world.matching_engine.order_book().order_count(), 0U);
        ...
        TEST(TradingRequestAgentExecutionAdapterTest,
            ...
            EXPECT_EQ(
                ...
                AgentActionResult{CancelActionResult{AgentCancelStatus::NotOwner}});
            ...
            EXPECT_EQ(
                world.actions.execute(
                    agent_a,
                    CancelOrderAction{order_id}),
                AgentActionResult{CancelActionResult{AgentCancelStatus::Cancelled}});
        ...
        TEST(TradingRequestAgentExecutionAdapterTest,
            ...
            const AgentActionResult agent_result = world.actions.execute(
                agent_a,
                SubmitOrderAction{Side::Buy, 90, 1});
            const TradingResponse execution_result =
                world.request_executor.execute(
                TradingRequest{
                    700,
                    account_b,
        ...
        TEST(AgentPolicyTest, DeterministicallyBuysAtEligibleAskOtherwiseHolds) {
            ...
            const AgentDecisionProvider& provider = policy;

            AgentObservation no_ask{};
            EXPECT_EQ(provider.decide(no_ask), AgentAction{HoldAction{}});
            no_ask.world.internal_market.best_ask = 101;
            EXPECT_EQ(provider.decide(no_ask), AgentAction{HoldAction{}});
            no_ask.world.internal_market.best_ask = 100;
            EXPECT_EQ(
                provider.decide(no_ask),
        ...
        TEST(AgentFoundationIntegrationTest,
            ...
            const AgentObservation observation =
                world.observations.observe(
                    agent_a,
                    world.observations.capture_world(1));
            ASSERT_EQ(observation.world.internal_market.best_ask, 100);
        ...
        TEST(AgentFoundationDeterminismTest,
            ...
            EXPECT_EQ(
                first.observations.observe(
                    agent_a,
                    first.observations.capture_world(1)),
                second.observations.observe(
                    agent_a,
                    second.observations.capture_world(1)));
            EXPECT_EQ(

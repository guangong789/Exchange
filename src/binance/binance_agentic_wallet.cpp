...
namespace exchange {
    ...
    ExternalPaymentPreview BinanceAgenticWalletCliBridge::preview(
        const ExternalPaymentRequirement& requirement) {
        return preview_with_evidence(requirement).preview;
    }

    BinanceAgenticWalletPreviewEvidence
    BinanceAgenticWalletCliBridge::preview_with_evidence(
        const ExternalPaymentRequirement& requirement) {
        ...
        const std::string wallet_status = require_string_member(status_data, "status");
        ...
        const Json preview_response = run_baw(
            ...
            {"x402-payment",
             ...
             "--json"});
        return {wallet_status, normalize_preview(preview_response, requirement)};
    }
